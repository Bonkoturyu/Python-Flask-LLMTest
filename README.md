...
このコマンドは [requirements.txt](requirements.txt) を読み、Flask、Ollama SDK、OpenAI SDK、Google Gen AI SDK、Anthropic SDKなどをプロジェクト専用環境へインストールします。
...
| `app.min_request_interval_seconds` | 同じブラウザーからLLMへ送信できる最短間隔。`0`で無効 | `1.0` |
...
| `llm.anthropic.api_key` | Anthropic APIキー | `sk-ant-...` |
| `llm.anthropic.base_url` | Anthropic APIの接続先 | `https://api.anthropic.com` |
| `llm.gemini.api_key` | Google Gemini APIキー | APIキー文字列 |
| `llm.openai.api_key` | OpenAI APIキー | `sk-...` |
...
Ollamaを動かすPCにOllamaをインストールします。[Ollama Windows版のダウンロードページ](https://ollama.com/download/windows)からインストーラーを利用できます。

PowerShellからインストールする場合:
...
LM Studio 0.4.0以降でAPI Token認証を有効にした場合は、「Developer」→「Server Settings」→「Manage Tokens」でTokenを作り、`config.local.yaml` に設定します。

```yaml
lmstudio:
  base_url: http://localhost:1234/v1
  api_key: ここにLM StudioのAPI Token
```

環境変数 `LM_STUDIO_API_KEY` または `LM_API_TOKEN` でも指定できます。詳しくは[LM Studioの認証公式資料](https://lmstudio.ai/docs/developer/core/authentication)を参照してください。
...
llm:
  ...
  anthropic:
    api_key: sk-ant-ここに実際のAPIキー
    base_url: https://api.anthropic.com
```

APIキーを設定ファイルへ書かず、PowerShellの環境変数で渡すこともできます。

```powershell
$env:ANTHROPIC_API_KEY = "sk-ant-ここに実際のAPIキー"
```

環境変数を使う場合、YAML側は空欄にします。
...
llm:
  ...
  gemini:
    api_key: ここにGemini APIキー
```

APIキーをYAMLへ書かず、環境変数で設定することもできます。

```powershell
$env:GEMINI_API_KEY = "ここにGemini APIキー"
```

環境変数を使う場合、YAML側は空欄にします。`GOOGLE_API_KEY` にも対応しています。
...
```powershell
$env:OPENAI_API_KEY = "ここにOpenAI APIキー"
```
...
`config.local.yaml` だけでなく、`config.test.yaml` のような `config.*.yaml` もGit管理から除外されます。公開用の `config.example.yaml` だけが例外です。設定ファイル名にかかわらず、秘密情報を含むファイルを `git add -f` で強制追加しないでください。

## テストを実行する

開発時の動作確認用に自動テストが用意されています。

初回だけ `pytest` をインストールします。
...
- `config.local.yaml` や `config.test.yaml` など、実環境の設定ファイルはGitへ追加しないでください。
- APIキーを `config.example.yaml` やREADMEへ記載しないでください。
- Flaskの開発サーバーをそのままインターネットへ公開しないでください。
- アプリにはCSRF対策、簡易的な連続送信制限、リクエスト容量制限がありますが、利用者認証はありません。本番公開時はHTTPS、利用者認証、永続ストレージを使うレート制限を追加してください。
- `FLASK_SECRET_KEY` が未設定の場合は起動時にランダム生成され、警告ログが表示されます。これは単一プロセスの開発用です。複数ワーカーではセッションが失効しないよう、十分に長い同一のランダム値を全ワーカーの環境変数へ設定し、共有しないでください。
